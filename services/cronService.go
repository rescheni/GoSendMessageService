...
func Cron_set(c *gin.Context) {
	...
	// 检查是否存在重复的 EntryID
	inCron.CronName = req.CronName // 任务名称
	...
	switch req.TaskType {
	case "wxpusher":
		err = basic.SetCronTask(req.CronName, req.CronExpr, func() {
...
func Cron_update(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid entryid/id format"})
		return
	}
	// 根据任务类型更新不同的定时任务
	switch req.TaskType {
	case "wxpusher":
		if basic.UpdateCronTask(cron.EntryID(entryID), req.CronName, req.CronExpr, func() {
			sendserver.SendWxPusher(req.Title, req.Message)
		}) {
	...
	case "napcat_qq":
		if basic.UpdateCronTask(cron.EntryID(entryID), req.CronName, req.CronExpr, func() {
			sendserver.SendQQPrivateMsg(req.Message, req.ToUser)
		}) {
			c.JSON(http.StatusOK, gin.H{"message": "Cron task updated successfully"})
		} else {
			c.JSON(http.StatusNotFound, gin.H{"error": "Cron task not found"})
